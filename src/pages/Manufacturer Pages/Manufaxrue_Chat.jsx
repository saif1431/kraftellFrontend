...
export default function ChatManufacture() {
  ...
  useEffect(() => {
    const fetchbuyer = async () => {
      try {
        const res = await axios.get(`${BASE_URL}/api/auth/buyers`, {
          headers: { Authorization: `Bearer ${token}` },
        });

        const filtered = Array.isArray(res.data)
          ? res.data.filter((user) => user.role === "buyer")
          : [];
      ...
      <div className="flex flex-1 overflow-hidden">
            ...
            contacts.map((contact) => (
              ...
              >
                <img
                  ...
                  alt={contact.businessName}
                  className="w-10 h-10 rounded-full object-cover"
                />
                <div>
                  <div className="font-semibold">{contact.businessName}</div>
        ...
        <div className="flex flex-1 flex-col">
          <div className="border-b border-gray-200 p-4 font-medium">
            {activeContact?.businessName || "Select a manufacturer"}
          </div>
